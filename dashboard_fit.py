...
NON_DATE_VALUES = {'HPH','Alta Adm.','MAL PREPARADO','Mal Preparada','NSP','PASIVO','Particular','RECHAZA COLONOSCOPIA','Suspendida',''}

def has_colono(df_sub):
    def es_fecha(v):
        if pd.isna(v): return False
        if isinstance(v, str): return v.strip() not in NON_DATE_VALUES
        return True
    return df_sub['F. COLONOSCOPIA'].apply(es_fecha)
...
con_res  = pos_n + neg_n
col_pos  = int(has_colono(df[df['fit_pos']]).sum())
col_neg  = int(has_colono(df[df['fit_neg']]).sum())
pend_pos = int((~has_colono(df[df['fit_pos']])).sum())
pend_neg = int((~has_colono(df[df['fit_neg']])).sum())
